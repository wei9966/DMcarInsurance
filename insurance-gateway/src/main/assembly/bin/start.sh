...
cd ..
DEPLOY_DIR=`pwd`
SERVER_NAME=insurance-gateway
JAR_File=$DEPLOY_DIR/lib/insurance-gateway-1.0-SNAPSHOT.jar
...
JAVA_DEBUG_OPTS=""
if [ "$1" = "debug" ]; then
    JAVA_DEBUG_OPTS=" -Xdebug -Xnoagent -Djava.compiler=NONE -Xrunjdwp:transport=dt_socket,address=80,server=y,suspend=n "
fi
JAVA_JMX_OPTS=""
if [ "$1" = "jmx" ]; then
